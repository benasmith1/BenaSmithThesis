...
```{r}
eu_orphan_drugs <- read_xlsx("Cleaned Data/Area Level Orphan Authorizations/NEWCOPYEU_Orphan_w_Company from https___www.orpha.net_orphacom_cahiers_docs_GB_list_of_orphan_drugs_in_europe.xlsx")
...
```{r}
stargazer(eu_ma_freq[1:9,], title= "EU Market Authorization Frequencies for Orphan Drugs with Over 1 Market Authorization", rownames = FALSE, align=TRUE, summary=FALSE, type = "html", out="Final Figures Formatted/EU_OD_Multiple_MAs.htm")
...
us_company_freq <- us_orphan_drugs %>%
  ...
  arrange(desc(n))
colnames(us_company_freq) <- c("Sponsor Company", "Market Authorization Frequency")
us_company_freq
```
...
Limit dataframe to 2004 and later, make lagged R&D spending NA at year
2004
...
ggplot(combined_rd_ma_df, aes(x = freq_new_orphan_drug_mas, y = annual_domestic_RD_spending_bil_dollars, color = area)) +
  geom_line() +
  labs(x = "Domestic R&D Spending",
       y = "Freq MAs for New Orphan Drugs",
       color = "Area",
...
actual <- combined_rd_ma_df$annual_domestic_RD_spending_bil_dollars
mod.rd.5.rmse <- Metrics::rmse(actual=actual, predicted=predicted)
...
stargazer(mod.rd.1, mod.rd.2, mod.rd.3, mod.rd.4, mod.rd.5, mod.rd.6, 
          ...
          covariate.labels = c("US", "Annual GDP Growth Rate Per Capita", "Years After 2004", "Years After 2004^{2})", "US:Years After 2004", "US:Years After 2004^{2}", "Intercept"),
...
To add Beta coefficients: (β<sup>̂</sup><sub>2</sub
...
actual <- combined_rd_ma_df$freq_orphan_drug_mas
mod.od.1.rmse <- Metrics::rmse(actual=actual, predicted=predicted)
...
actual <- combined_rd_ma_df$freq_orphan_drug_mas
mod.od.2.rmse <- Metrics::rmse(actual=actual, predicted=predicted)
...
actual <- combined_rd_ma_df$freq_orphan_drug_mas
mod.od.3.rmse <- Metrics::rmse(actual=actual, predicted=predicted)
...
actual <- combined_rd_ma_df$freq_orphan_drug_mas
mod.od.4.rmse <- Metrics::rmse(actual=actual, predicted=predicted)
...
actual <- combined_rd_ma_df$freq_orphan_drug_mas
mod.od.5.rmse <- Metrics::rmse(actual=actual, predicted=predicted)
